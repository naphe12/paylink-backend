...
Create Date: 2026-03-28 15:30:00
"""
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        ON ia.ai_pending_actions (user_id, status, expires_at DESC)
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_ai_audit_logs_user_created_at
        ON ia.ai_audit_logs (user_id, created_at DESC)
        """
    )
    op.execute(
        """
    ...
    op.execute(
        """
        DELETE FROM ia.ai_intent_slots
        WHERE intent_code IN ('agent_onboarding.guide', 'agent_onboarding.scenario', 'cash.deposit', 'cash.withdraw', 'wallet.balance', 'wallet.block_reason', 'wallet.limits', 'credit.capacity', 'credit.simulate_capacity', 'credit.pending_reason', 'transfer.create', 'beneficiary.add')
        """
    )
    op.execute(
        """
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS paylink.uq_external_beneficiaries_user_partner_phone_account")
    op.execute("DROP TABLE IF EXISTS paylink.external_beneficiaries")
    op.execute("DROP INDEX IF EXISTS ia.uq_ai_synonyms_tuple")
    op.execute("DROP INDEX IF EXISTS ia.idx_ai_audit_logs_user_created_at")
    op.execute("DROP INDEX IF EXISTS ia.idx_ai_pending_actions_user_status")
    op.execute("DROP TABLE IF EXISTS ia.ai_audit_logs")
    op.execute("DROP TABLE IF EXISTS ia.ai_conversation_state")
    op.execute("DROP TABLE IF EXISTS ia.ai_pending_actions")
    op.execute("DROP TABLE IF EXISTS ia.ai_action_registry")
    op.execute("DROP TABLE IF EXISTS ia.ai_synonyms")
    op.execute("DROP TABLE IF EXISTS ia.ai_intent_slots")
    op.execute("DROP TABLE IF EXISTS ia.ai_intents")
    op.execute("DROP SCHEMA IF EXISTS ia")
