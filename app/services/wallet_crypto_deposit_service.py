...
def get_paylink_deposit_address(token_symbol: str, network: str) -> str:
    ...
    if normalized_address == "0x0000000000000000000000000000000000000000":
        raise ValueError(f"PesaPaid deposit address for {normalized_token} is not configured")
...
async def _send_admin_deposit_email(payload: dict) -> None:
    ...
    try:
        ...
        html = (
            ...
            f"<p>Adresse PesaPaid: {payload['deposit_address']}</p>"
            f"<p>Tx hash: {payload['tx_hash']}</p>"
        )
        await run_in_threadpool(
            mailer.send_email,
            admin_email,
            f"[PesaPaid] Depot {payload['token_symbol']} detecte",
            None,
            body_html=html,
            text=(
